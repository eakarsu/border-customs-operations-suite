const baseUrl = process.env.SMOKE_BASE_URL || 'http://127.0.0.1:4910';
...
const adminCookie = await login('admin@border-customs-operations.local', 'admin123');
const managerCookie = await login('manager@border-customs-operations.local', 'manager123');
const analystCookie = await login('analyst@border-customs-operations.local', 'analyst123');
